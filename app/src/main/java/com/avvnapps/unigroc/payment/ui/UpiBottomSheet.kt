package com.avvnapps.unigroc.payment.ui
...
import com.avvnapps.unigroc.payment.adapter.UpiListAdapter
...
class UpiBottomSheet : BottomSheetDialogFragment(), AnkoLogger {
    ...
    override fun getTheme(): Int {
        return R.style.upi_style_Dialog_BottomSheet
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        mUri = Uri.parse(requireArguments().getString("uri"))
        mUpiApps = requireArguments().getStringArrayList(UpiPayment.ARG_UPI_APPS_LIST)!!
    }
        ...
        if (mUpiApps.isNotEmpty()) {
            ...
            for (i in appList) {
                if (mUpiApps.contains(
                        i.loadLabel(requireContext().packageManager).toString().toLowerCase()
                    )
                )
                    finalList.add(i)
            }
